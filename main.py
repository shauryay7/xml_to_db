...
def get_custom_timestamp():
    now = datetime.utcnow()
    # Format: YYYYY-Mmm-SddTHH:MM:SS.mmmZ
    # %f gives microseconds, slice last 3 digits for milliseconds
    return now.strftime(f"Y%Y-M%m-S%dT%H:%M:%S.%f")[:-3] + "Z"
...
def insert_full_entity_data(xml_file, db_name="AutoGeneratedDB"):
    ...
    for entity in root.findall(".//Entity"):
        ...
        if table_name:
            # Add timestamp before inserting/updating
            entity_doc["inserted_at"] = get_custom_timestamp()
            ...
            print(f"\n✅ MongoDB: Inserted/Updated schema for `{table_name}`")
            print("📝 Inserted Document:")
            pprint(entity_doc)
            inserted_count += 1

    print(f"\n✅ Done: {inserted_count} entity schemas inserted into `{db_name}.schema_definitions`")
